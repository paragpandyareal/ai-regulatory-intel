...
binding=must/shall/required to, guidance=should/may/recommended, definition=means/refers to, example=illustrative.`,
    undefined,
    1000
  );
...
export async function classifyObligations(
  ...
  for (let i = 0; i < obligations.length; i += batchSize) {
    ...
    console.log('[Classify] Batch ' + (Math.floor(i / batchSize) + 1) + '/' + Math.ceil(obligations.length / batchSize));
